...
import type { MobileOrder, OrderStatus } from '../services/orders.service'
...
export default function OrderCard({ order, onPress }: Props) {
  const config = STATUS_CONFIG[order.status] ?? { label: order.status, color: '#374151', bg: '#f3f4f6' }
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={() => onPress(order.id)} activeOpacity={0.7}>
      ...
      <Text style={styles.name} numberOfLines={2}>{order.name}</Text>
      <View style={styles.footer}>
        <Text style={styles.secretary} numberOfLines={1}>{order.secretary?.name ?? '—'}</Text>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 14, color: '#374151', lineHeight: 20, marginBottom: 10 },
  footer: { flexDirection: 'row', justifyContent: 'space-between' },
  secretary: { fontSize: 12, color: '#6b7280', flex: 1, marginRight: 8 },
